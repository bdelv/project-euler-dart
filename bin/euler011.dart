  ...
  for (int x = 0; x < gridSize; x++) {
    for (int y = 0; y < gridSize; y++) {
      ...
      if (x + adjacentNumbersCount <= gridSize) {
        tempList.clear();
        for (int i = 0; i < adjacentNumbersCount; i++) tempList.add(Grid[x + i][y]);
        testList();
      }
      //vertical
      if (y + adjacentNumbersCount <= gridSize) {
        tempList.clear();
        for (int i = 0; i < adjacentNumbersCount; i++) tempList.add(Grid[x][y + i]);
        testList();
      }
      //diagonal 1
      if ((x + adjacentNumbersCount <= gridSize) && (y + adjacentNumbersCount <= gridSize)) {
        tempList.clear();
        for (int i = 0; i < adjacentNumbersCount; i++) tempList.add(Grid[x + i][y + i]);
        testList();
      }
      //diagonal 2
      if ((x + adjacentNumbersCount <= gridSize) && (y + adjacentNumbersCount <= gridSize)) {
        tempList.clear();
        for (int i = 0;
            i < adjacentNumbersCount;
            i++) tempList.add(Grid[x + i][y + adjacentNumbersCount - i - 1]);
        testList();
      }
    }
  }
